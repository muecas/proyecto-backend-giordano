...
routes
	...
	.post('/productos', async (request: Request, response: Response) => {
		...
		if(result === false) {
			response.json({ error: 'Ha ocurrido un error al crear el registro en la base de datos' })
		} else if(result.hasOwnProperty('errors') === true) {
			response.json({ error: 'Verifica los campos ingresados', errors: (result as ValidationResult).errors })
		} else {
...
io.on('connection', async (socket: Socket) => {
	...
	await messages.getAll().then(messages => socket.emit('messages', { messages }));
	socket.on('message', async ({ email : from, message }) => {
		await messages.create({ from, message });
		messages.getAll().then(messages => io.sockets.emit('messages', { messages }));
	});
});